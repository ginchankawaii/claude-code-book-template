...
def build_candidates(kabu: KabuStation, watch: list[str], held: list[str], n: int) -> list[dict]:
    pool: list[str] = []
...
def cycle(kabu: KabuStation, trader: AIStockTrader, max_risk: float, max_positions: int,
          watch: list[str], n_candidates: int, trigger: str) -> bool:
    ...
    candidates = build_candidates(kabu, watch, held_syms, n_candidates)
    ctx = {
...
def main() -> None:
    ...
    if args.once:
        cycle(kabu, trader, args.max_risk, args.max_positions, watch, args.candidates, "manual")
        return
