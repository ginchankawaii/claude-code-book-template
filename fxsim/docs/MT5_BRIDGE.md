> **⚠️ 歴史的文書（Round-6）**：この `run_bridge` はAI無しの旧ブレインで、現行のブリッジ規約（SEQ/EXP/SL）を
> 話さずロックも取りません。**AI版（Docker の fx）が動いている間は起動を拒否します**（`--dry` は可）。
> Task Scheduler に "SteadyBridge" 系のジョブが残っていたら**削除**してください——Round-6で
> 「忘れられたジョブが第2のライターになる」経路として確認されました。
