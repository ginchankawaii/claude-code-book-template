...
def test_last_close_independent_of_window():
    # 要求日数が変わっても最新終値（=現在値）は一致する
    short = market.get_history("7203", days=10)
    long = market.get_history("7203", days=120)
    assert short[-1]["close"] == long[-1]["close"]
    assert short[-1]["date"] == long[-1]["date"]
    assert market.get_quote("7203")["price"] == long[-1]["close"]
