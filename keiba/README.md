...
> **デモの回収率(>100%)を実力値・下限値と読まないでください**: EVフィルタの
> 選択バイアスと少フォールド分散で大きく上振れし、真のエッジが無い設定でも100%
> 超が出ることがあります。実データでは控除率の壁(単複20%)で回収率は大きく
> 下がるのが現実です。値は単点でなく分布(複数シード)で見るべきです。
...
python -m keiba --myopia 0.3     # ほぼ効率的: ブレンドは市場の log-loss を下回らない
python -m keiba --myopia 0.9     # 近視眼的市場: ブレンドが市場を明確に上回る
