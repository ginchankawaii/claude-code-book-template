...
def cmd_threshold(args):
    ...
    print()
    # 出走馬が欠けていると市場確率も倍率も過大／過小になる。
    # 実効控除率が想定から外れていれば、まず入力を疑う。
    if not (0.15 <= to <= 0.32):
        print(f"【警告】実効控除率 {to*100:.1f}% は正常な範囲（15〜32%）を外れています。")
        print("  出走馬のオッズが欠けているか、券種の取り違えが疑われます。")
        print("  以下の市場確率・必要確率はいずれも信頼できません。")
        print()
