...
| 項目 | 値 |
|---|---|
| 電池 | **LiPo 802530（3.7V 680mAh、31×25×8.0mm、保護回路付き。採用品: HXJNLDC、[`bom.md`](bom.md) #2）** |
| 外形 | **39.0 × 49.4 × 14.6 mm** |
| 印刷部品の重量 | 15.3 g（body 7.9 / lid 4.6 / backplate 2.9） |
| デバイス総重量 | **約34 g**（採用品電池の公称11g。[§9](#9-既知の課題未検証事項)の注記も参照） |
| 素の状態（省電力実装なし）の連続録音 | **約10.6時間** |
...
| allday | 680mAh | 578mAh | **約10.6時間** |

「起床から就寝まで（約15時間）」を目標にすると、**どちらのバリアントも素の状態では届きません**。
allday でも worst case は10.6時間ですが、ファームウェア側の省電力
（CPUクロックを240MHz→80MHzに下げる／SDへの書き込みをRAMにためてまとめ書きする／
無音を検出して書き込みを止める VAD ゲート）で**平均28mAの目標（[§8.7](#87-テスト6-消費電流の実測ゲート必須)）を達成すれば約20.6時間**が見込めます。

そして厚みの増加は **1.7mm だけ**です。電池は 5.0mm → 8.0mm と3mm厚くなりますが、
筐体の奥行きを決めているのは主に XIAO のスタック（基板＋拡張ボード）側なので、外形にはそのまま反映されません。
**この1.7mmと引き換えに録音時間が約1.4倍になるので、迷ったら allday** を選んでください。

> **slim を選ぶ理由があるとすれば**: 「とにかく薄くしたい」「会議の1〜2時間だけ使えればいい」場合です。

> **重要な注意（充電時間）**: XIAO の充電電流は **50mA固定**です。680mAh を空から満充電にするには
> **15〜16時間**かかります（500mAhでも11〜12時間）。つまり **一晩（10時間）の充電で戻せるのは約500mAh**。
> allday の 680mAh は「1日にたくさん使うための容量」ではなく、**省電力実装が間に合うまでのマージン・
...
- **サイズは型番がそのまま寸法です**（例: 802530 = 厚8.0 × 幅25 × 長30 mm。ただし採用品の実仕様は全長31mm）。
  **容量は型番に含まれません** — 802530 の実容量は約680mAhで、「800mAh」を名乗る 802530 は
  物理的に不可能な密度です（[`bom.md`](bom.md) のエネルギー密度検算を参照）。
  - allday を作る → **802530（680mAh。採用品は [`bom.md`](bom.md) #2 の HXJNLDC）**
...
- そこに安全代を見た目標値が **28mA**。
- **これはバリアントによらず同じ**です（allday の680mAhは「1日の上限」ではなく余力・劣化代なので）。
...
- **それでも足りない場合**、slim を作っていたなら **allday へ切り替え**ます
  ...
  なお allday の680mAhは「1日の消費上限を上げるもの」ではない（一晩に戻せるのは500mAhのまま）ので、
...
| **802530（680mAh）電池の実寸** | 31×25×8.0mm、保護回路付き（採用品 HXJNLDC のメーカー仕様表・商品画像で確認済み） | **仕様表確認済み・現物未着**。届いたらノギスで実寸を測り、黄色テープ下の保護基板を目視確認すること（[`bom.md`](bom.md) #2） |
...
- したがって **680mAh を空から満充電にするには15〜16時間**かかります（500mAhでも11〜12時間）。
- **一晩10時間の充電で戻せるのは約500mAh**。
- **つまり、allday の680mAhにしても「1日に使える電力量」は増えません。**
...
- **採用品 HXJNLDC 802530 の公称重量は11g**（メーカー仕様表）で、その場合の総重量は **約34g**になります（**現物未実測**）。
