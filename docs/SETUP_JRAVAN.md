...
```python
# jrvltsql の既定出力は SQLite(data/keiba.db, テーブル NL_SE/NL_RA/NL_O1)。
from keiba.ingest import from_sqlite, validate_runners   # DuckDB なら from_duckdb
...
runners, races = from_sqlite(r"jrvltsql\data\keiba.db")

# 2) まず健全性チェック(列名違い=全NaN、着順重複、オッズスケール等を自動診断)
issues = validate_runners(runners)
print(issues or "クリーン")            # ← 何か出たら、その1行を私に送ってください
