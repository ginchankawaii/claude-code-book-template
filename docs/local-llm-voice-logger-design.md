...
- 3Dプリント（クリップ or ネックレス型）。Omi の公開STLが流用可能
- 物理スライドスイッチを必ず設ける（後述のプライバシー/法務面で重要）
- マイク開口部 + 録音中LEDインジケータ
...
| Phase | 内容 | ポイント |
|---|---|---|
| **0. パイプライン検証（最優先）** | 古いスマホやICレコーダーで録音した音声を、サーバ側パイプライン（VAD→whisper→pyannote→Ollama→Obsidian）に流す | **ハードより先にソフトを完成させる**。既存のObsidian音声日記システムと出口を統合 |
