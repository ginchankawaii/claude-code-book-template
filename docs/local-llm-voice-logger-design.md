...
- 録音中（WiFi/BLE オフ、SD書き込みあり）：**Seeed公式スペック表の実測値で 54.4mA 平均**
  （ピーク 108mA）。省電力実装（CPU 80MHz化・SDバッファ書込・VADゲート）を入れた
  上での**目標が平均 28mA**で、これは実測ゲートで担保する（`hardware/ELECTRICAL.md` §2.2）
- **LiPo 500mAh (slim)** → 省電力実装なしの worst case で約7.8時間。
  **802530 (680mAh / allday) が既定**で、worst case で約10.6時間・ゲート通過時20.6時間
- 同期時のみ WiFi ON（数分間、~100mA）なので影響は軽微
- 薄型 LiPo 502530（500mAh、約10g）で総重量 20g 前後を狙える
- **ゼロタッチ要件の含意：バッテリーは「起床〜就寝」を必ず走り切ること**。
  装着時間が15時間を超える生活パターンなら 802530（680mAh）を選ぶ。
  （**703040 は筐体の高さに収まらないので不可**。30×25mm のフットプリントを保ち、
  厚みだけが 5.0→8.0mm 変わる 802530 を使う）。あるいは
