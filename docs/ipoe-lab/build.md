...
> **⚠ この値はラボ専用です。実回線の機器に入れないでください。**
> 実サービスの MAP ルールは VNE ごとに違い、`draft-ietf-softwire-map-03` ベースの
> 独自パラメータで配布されます。**ここで手入力に慣れた値が、そのまま現場に転写される事故**を
> 防ぐため、案件では必ず事業者の提供値を使ってください。逆に、**実サービスの値をラボに
> 混ぜるのも禁止**です(このラボの整合が崩れて、以降の検証が全部あてになりません)。
...
| ra(ひかり電話なし) | `2001:db8:1014:300::/64` | `198.51.100.20` | 3 | `2001:db8:1014:300:0:c633:6414:3` | `CE_MAP_ADDR=2001:db8:1014:300:0:c633:6414:3 CE_SHARED_V4=198.51.100.20 CE_PSID=3` を付けて実行 |
