...
**発生タイミング(1 秒の分解能では切り分け不能)**

| 時刻 | 出来事 |
|---|---|
| 11:47:54 | 設定投入完了(`%SYS-5-CONFIG_I`) |
| **11:47:55** | **QFP ucode のコア生成** |
| 11:48:13 | HA が「スタックスレッド」を検出して FATAL 宣言(syslog に出るのはここ) |

**設定完了の 1 秒後**に落ちている。ただし `ping -t` を流しっぱなしだったため、
**「設定反映の瞬間」か「反映後の最初のパケット」かは区別できていない。**

> **次回の切り分け実験(安価かつ決定的)**: トラフィックを一切流さない状態で
> MAP-E を設定する。落ちれば設定起因、落ちなければパケット起因。

**引き金の候補**: `ipv4-prefix` を `/24`(ルール全体)から **`/32`(CE の共有 IPv4)**に
変えた設定でのみ落ちた。`/24` の誤設定のままでは(変換自体が起きないので)落ちなかった。
**つまり「正しく設定したときだけ落ちる」**。
