...
> リポジトリはホストの `/root/ipoe-lab` に tar で展開してある。
> スクリプトを直したら**ホスト側も更新する**こと(`git pull` ではなく tar で取り直す。ホストに git は無い)。
...
| 2 | 4 台の Linux に setup スクリプトを流し、IPv6 が降りるところまで | **Plan 済**(次はこれ) | — | — |
