...
| 8 | C1111-8P(中古)の受け入れ評価 | **完了**(判定基準 8 件すべて達成) | 約 60 分 | 出品情報がシリアルを伏せていても、**ACK 期限と flash のタイムスタンプの一致**で個体を同定できる(46 秒差) |
| 9 | C1111-8P 実機で MAP-E を検証 | **完了**(ただし結論は誤りだった) | 約 180 分 | 手動 BMR で **6/6 クラッシュ**。ここで「機種が使えない」と結論しかけた。**バージョン・IPv6 の配布方式・IPv4 の共有有無を変えても同じ**だったことが、逆に「設定値ではなく経路の問題」を示していた |
| 10 | MAP-E の設定経路が間違っていたと判明 | **完了**(サイクル 9 の結論を撤回) | 約 60 分 | **一次情報を最初に読む。**測っていた `basic-mapping-rule` 手書きは **Cisco のドキュメントに無い経路**だった。正規は `jp01` によるルール配布サーバからの取得 |
| 11 | ルール配布サーバをラボに建てる(設計不良の是正) | **完了** | 約 90 分 | **ラボに本番の構成要素が丸ごと欠けていた。**個々の手順をいくら検証しても、**欠けていること自体は見つからない**。仕様(HB46PP)は JAIPA が公開しており、推測で作らずに済んだ |
| 13 | 固定IP1(share-ratio 1)の実機検証 / ポート枯渇の自然発生 | **完了** | 約 30 分 | **配る数字ひとつ**(`eaBitLength` 16→8)で 240 ポート → 64512 ポートに変わる。「固定IPオプション」の正体がこれ。**一度受理したルールは reload しないと取り直さない。**そして **240 個は PC 1 台の放置で枯れる**(仕込んでいないのに `%NAT-6-ADDR_ALLOC_FAILURE` が出た) |
| 12 | C1111-8P で本番経路の MAP-E を通す | **完了**(`src 198.51.100.20` を実測) | 約 120 分 | **サイクル 9 の「この機種は使えない」は完全な誤りだった。**原因は一つで、本番に無い経路を測っていたこと。応答フォーマットは一次情報に無かったが、**実機に投げれば当たりか分かる**。CE の MAP アドレスは **RFC 7597 と並びが違う**(draft-03)。「ping が飛ばない」の正体が **PC の Wi-Fi 併用**だったのは、切替当日に必ず踏む類 |
...
`nat64 provisioning mode jp01`(日本の VNE 向けモード)も存在した。
CLI 構造の全数調査とラボ設計値の翻訳は [c1111-acceptance.md §7](c1111-acceptance.md) に記録。
...
### サイクル 9: C1111-8P 実機で MAP-E を検証(2026-08-13)

**目的**: バックログ 10(MAP-E の実効ポート数)を実機で決着させる。
**結果**: **バックログ 10(実効ポート数)は測れず。**ただし**それより重い結果**が出た —
**IOS XE の MAP-E は実機でも QFP がクラッシュして使えない**ことが確定した。

#### 分かったこと(いずれも実機で確認)

**① 【最重要】実機でも QFP がクラッシュした。MAP-E は IOS XE で使えない**

**この検証で一番重い結果。**サイクル 7 で Cat8000v(仮想)が落ちたとき、
「仮想固有かもしれない」という留保を付けていた。**実機で確認した結果、留保は不要だった。**

```
%CPPHA-3-FAILURE: F0/0: cpp_ha_top_level_server: CPP 0 failure Stuck Thread(s) detected
%CPPHA-3-FAULT:   CPP:0.0 desc:Stuck CPP Thread det:HA class:OTHER sev:FATAL cppstate:STOPPED
...
| 環境 | IOS XE | 結果 |
|---|---|---|
| CML Cat8000v(仮想) | 17.15.01a | **QFP クラッシュ** |
| **実機 C1111-8P** | **17.15.05** | **QFP クラッシュ** |

**仮想でも実機でも、別バージョンでも、NAT64 データパスの同じ場所で落ちる。**

**クラッシュダンプを採取済み**(受け入れ評価時は `core/` が空だったので、
個体不良ではなく MAP-E 設定が原因であることが切り分けられている):

```
CPE_RP_0_qfp-ucode-tsn-le_15283_20260813-114755-UTC.core.gz    21.9 MB
CPE_RP_0_cpp_cp_svr_16368_20260813-114822-UTC.core.gz          15.5 MB
CPE_RP_0-system-report_20260813-115102-UTC.tar.gz              (TAC 提出用)
```

#### 切り分け実験 — 引き金は「設定」ではなく「パケット」(同日、実施済み)

**やり方**: LAN ポートを `shutdown` してトラフィックをゼロにした状態で、
MAP-E に必要な設定を**1 つずつ**足し、各段階で 60 秒待って観察した。

| 段階 | 投入したもの | 結果 |
|---|---|---|
| 1 | WAN を up + `ipv6 dhcp client pd`(/56 取得) | 無傷 |
| 2 | WAN に MAP-E アドレス `/128` | 無傷 |
| 3 | LAN に IPv6(委譲 /56 の 2 番目の /64) | 無傷 |
...
| **8** | **LAN ポートを `no shutdown`(= PC を接続)** | **12:30:16 にクラッシュ** |

**結論: 設定しただけでは落ちない。パケットが流れると落ちる。**

**しかも `ping` は打っていない。**LAN ポートが up になり、PC が繋がった瞬間に出す
通常のトラフィック(ARP / DHCP / mDNS 等)だけで落ちた。

**タイミングは 2 回とも同じパターン**(引き金の 1 秒以内に死に、10〜20 秒後に HA が検出):

| | 引き金 | ucode コア生成 | HA が検出 |
|---|---|---|---|
...
```
CPE_RP_0_qfp-ucode-tsn-le_15213_20260813-123016-UTC.core.gz   24.2 MB
CPE_RP_0_cpp_cp_svr_16238_20260813-123038-UTC.core.gz         18.0 MB
```

**再現性 100%(2/2)。**

> **実務上の言い方**: 「MAP-E を設定した状態で LAN を繋いだら、その瞬間に全断する」。
> 何か特別な操作をする必要はない。**PC を挿すだけ。**

**未切り分けのまま残した点**: 「インタフェースが up になった瞬間のデータパス
プログラミング」か「up 後に届いた最初のパケット」かは、1 秒の分解能では区別できていない。
切り分けようとして LAN ポート同士をケーブルで折り返したところ、**同一 VLAN のため
ブロードキャストストームになり QFP 負荷 100% / ARP 186 kpps** になったので中止した
(受け入れ評価時の疎通テストでは Vlan1 に IP が無く通信も無かったため増幅しなかった。
**条件が違うことを見落とした**)。実務上の結論は変わらないため、これ以上は追わない。

#### 実務上の意味(これがこのラボを作った理由そのもの)

> **IOS XE 機を MAP-E 案件に提案するとき、「CLI があるから大丈夫」は通用しない。**
> 設定は通り、`show nat64 map-e` は正しい導出値を返し、
> **その状態で MAP-E を正しく設定した瞬間に転送エンジンが落ちて全断する。**
>
> **現場でこれが起きたら原因究明に何日かかるか分からない。**
> 事前に実機で、パケットを流すところまで検証すること。

**892FJ は MAP-E 非対応、CML の Cat8000v も実機 C1111-8P も QFP クラッシュ。**
**MAP-E の CE として現時点で使えるのは OpenWrt のみ。**実サービスの CE 検証には
RTX 等の別ベンダ機か、別バージョンの IOS XE が必要。
...
手で与えたのは `share-ratio 256` と `port-offset-bits 4` の 2 つだけ。
**`Contiguous-ports 16` と `Start-port 4096` は自動導出**され、手計算(15 × 16 = 240)と一致した。

**③ `nat64 provisioning mode jp01` が存在する**

日本の VNE 向けプロビジョニングモード。ただし**サブモードに入る**ので、
プロビジョニングサーバのない環境では**外したほうがよい**(手動設定と排他の可能性)。

**④ パラメータの投入順に依存がある ← 資料に必須**

```
port-set-id 3
%NAT64: PSID cannot be greater than the sharing ratio
```

**`port-parameters`(share-ratio)を先に入れないと `port-set-id` が弾かれる。**
share-ratio 未設定時は既定 1 と見なされるため。**上から順に打つと必ず踏む。**

**⑤ MAP-E アドレスは自動生成されない**

DHCPv6-PD で `/56` を取得しても、CE の MAP-E アドレス
(`2001:db8:100a:500:0:c633:640a:5`)は**どのインタフェースにも自動では付かない。**
手で `/128` を設定する必要がある。付けないと網側から NDP が引けず(`FAILED`)、
**戻りの通信が届かない。**

**⑥ CE アドレスは委譲 /56 の先頭 /64 に入る**

```
%Error: 2001:DB8:100A:500:0:C633:640A:5/128 is overlapping with
        2001:DB8:100A:500::/64 on Vlan1
```

**LAN に先頭 /64 を使うと IOS が重複を拒否する。**
LAN は 2 番目以降の /64 を使うこと(`ipv6 address LAB-PD 0:0:0:1::1/64`)。

**⑦ Cisco も DUID-LL を送る**

Kea のリースに記録された DUID は `00:03:00:01:2c:f8:9b:54:2b:80` = **DUID-LL**。
OpenWrt と同じ。**D1(DUID-LLT の罠)は実機 Cisco でも発動しない。**
参照 CE を変えても罠にならないので、test-matrix の D1 に注記が必要。

**⑧ DHCPv6-PD と復路は完全に成立した**

```
Prefix: 2001:DB8:100A:500::/56                                   (Kea から委譲)
2001:db8:100a:500::/56 via fe80::2ef8:9bff:fe54:2b80 dev ens19   (NGN-SIM の復路)
2001:db8:100a:500:0:c633:640a:5 lladdr 2c:f8:9b:54:2b:80 router  (NDP 解決)
```

**ラボの NGN-SIM が実機 Cisco 相手にも正しく振る舞うことが確認できた。**

#### 途中経過 — パラメータの意味を取り違えていた(解決済み)

```
show platform hardware qfp active statistics drop
  Nat64v4tov6    1535 packets      ← 増え続ける

show nat64 statistics
  Vlan1 ... Packets dropped: 1234  ← 増え続ける
  MAP-E: 0                          ← 変換はゼロ
```

**パケットは NAT64 プロセスまで届き、v4→v6 変換で落とされている。**
経路・インタフェース紐付け・アドレス・NDP はすべて成立しているので、
**残るのは変換ロジックのパラメータ解釈のみ。**

試した設定(いずれも `Nat64v4tov6` の増加が止まらず):

| # | 変更 | 結果 |
|---|---|---|
| A | `ipv6-prefix` をルールプレフィックス `2001:db8:1000::/40` | ドロップ継続 |
| B | `ipv6-prefix` を委譲プレフィックス `2001:db8:100a:500::/56` | ドロップ継続 |

**判明した正しいパラメータの意味(Cisco 公式の設定例より)**:

```
ipv4-prefix 209.165.200.225/32     ← CE の共有 IPv4 を /32 で。ルール全体ではない
local-ipv4-prefix 192.168.0.0/16  ← CE の内側(LAN)の私設アドレス
```

**この 2 つを取り違えていたため変換が起きず `Nat64v4tov6` で落ちていた。**
正しく直したところ、今度は**QFP がクラッシュした**(①を参照)。

**最終的に正しかった設定(ラボの値に当てはめたもの)**:

```
nat64 map-e domain 1
 border-relay-address 2001:DB8:9999::1
 basic-mapping-rule
  ipv6-prefix 2001:DB8:100A:500::/56        ← DHCPv6-PD で委譲された /56
  ipv4-prefix 198.51.100.10/32              ← この CE の共有 IPv4 (/32)
  local-ipv4-prefix 192.168.100.0/24        ← LAN
  port-parameters share-ratio 256 port-offset-bits 4
  port-set-id 5
```

**この設定を投入した 1 秒後に QFP がクラッシュした。**

> **教訓 1**: パラメータの意味が確定しないまま組み合わせを試すのは、時間の使い方として悪い。
> **CLI の `?` で構造は分かるが、意味は分からない。**最初にドキュメントを読むべきだった。
> 実際、公式の設定例 1 つで解決した。
>
> **教訓 2**: **「動かない」と「壊れる」は別物。**
> 誤った設定では「変換されずにドロップ」で済んでいたが、
> **正しい設定にした瞬間に転送エンジンごと落ちた。**
> 設定ミスに救われて重大な不具合が見えていなかった、という構図。

#### RA 方式では MAP-E が成立しない(仕様どおり)

最初 RA 方式(SLAAC /64)で試したが、`Vlan1 ... Packets dropped: 712` で変換ゼロだった。
**RFC 7597 の MAP-E は「委譲されたプレフィックス」が前提**なので、
SLAAC の /64 だけでは CE が自分のエンドユーザプレフィックスを確定できない。
**MAP-E の検証は PD 方式で行うこと。**

#### 運用上の記録

- **`vmbr1` の物理アップリンク(`nic0`)は実行時に手で足された状態**で、
  `/etc/network/interfaces` は `bridge-ports none` のまま。
  **Proxmox ホストを再起動すると外れる。**会社展開時の落とし穴。
- 892FJ が近隣テーブルに `router` として残っていた。
  **同時に繋ぐ CE は 1 台**というラボの原則を守ること。
- **ライセンス表示は設定に依存する。**受け入れ評価時は `appxk9` / `securityk9` が
  `IN USE` だったが、`write erase` + 最小構成でリロードしたら**両方 `None`** になった。
  SLP は「機能を設定したら使う」方式なので故障ではない(`unthrottled` は維持)。
  **受け入れ評価で書いた「IN USE 表示は権利の証明ではない」が裏付けられた形。**

#### 次サイクルでやること

1. **トラフィックなしで MAP-E を設定して、落ちるか確認**(設定起因かパケット起因かの切り分け)
2. **別バージョンの IOS XE で再現するか**(17.15 系以外)
3. バックログ 10(実効ポート数)は **OpenWrt でしか測れない**状態が続く。
   RTX 等の別ベンダ CE の入手を検討する
4. **説明会・ハンズオン資料への反映**。「IOS XE は MAP-E の CLI を持つが、
   17.15 系では QFP がクラッシュする」は**顧客提案に直結する事実**なので、
   資料の「892FJ は MAP-E 非対応」の記述と並べて載せる

### サイクル 10: MAP-E の設定経路が間違っていた(2026-08-13 夜)

**きっかけ**: 実案件(郡山の医療機関。OCN 光「フレッツ」IPoEワイド 固定IP1、
ひかり電話なし、CPE は C1111-8P)が、サイクル 9 で壊した構成そのものだと判明した。

#### 結論から: サイクル 9 の「RTX に切り替えるべき」は撤回

**サイクル 9 で測っていたのは、Cisco がドキュメント化していない設定経路だった。**

| | 私がやっていたこと | Cisco の正規の手順 |
|---|---|---|
| MAP ルール | **手動で `basic-mapping-rule` を全部書く** | **`jp01` で VNE のルール配布サーバから HTTPS 取得** |
| `provisioning mode jp01` | **「サーバが無いので邪魔」と判断して外した** | **これが本体** |
| `version draft-ietf-softwire-map-03` | 知らなかった | 必要 |
| `nat64 settings fragmentation header disable` | 入れていなかった | 必要(ただし 17.9.5f は既定で有効) |
| Root CA 証明書 | 存在を知らなかった | **`crypto pki trustpoint` で事前登録が必要** |

**`basic-mapping-rule` を手で書く方法は、Cisco のドキュメントに載っていない。**

#### 追加の実測(サイクル 9 の続き)

**IOS XE 17.9.5f に入れ替えて再試験**(顧客機は 17.9.2a = 同一トレイン)。
イメージは `verify` で署名まで確認済み。17.15.05 も bootflash に残してある。

| # | バージョン | IPv6 | IPv4 | 経路 | 結果 |
|---|---|---|---|---|---|
| 1-2 | 17.15.05 | PD | 共有(240 ポート) | 手動 BMR | **クラッシュ** |
| 3 | 17.15.05 | RA | 専有(固定IP相当) | 手動 BMR | **クラッシュ** |
| 4 | **17.9.5f** | RA | 専有 | 手動 BMR | **クラッシュ** |
| 5 | **17.9.5f** | RA | 専有 + フラグメント設定 | 手動 BMR | **クラッシュ** |
| **6** | **17.9.5f** | **RA** | — | **`jp01`** | **クラッシュしない** |

**手動 BMR は 5/5 でクラッシュ。`jp01` 有効化では落ちない。**

> **フラグメント仮説は否定された。**2 回目のクラッシュのトレースバックに `cpp_ipfrag_svr` が
> 出ていたので `nat64 settings fragmentation header disable` を疑ったが、
> **17.9.5f では既定で有効**であり、クラッシュ #4 の時点ですでに入っていた。

#### `jp01` の中身(次サイクルの設計図になる)

```
(config)# nat64 provisioning mode jp01
(config-nat64-provisioning)# ?
  address-resolution-server  nat64 address resolution server url
  api-key                    nat64 api key
  hostname                   hostname
  rule-server                nat64 rule server        ← ★ 向き先を指定できる
  service-prefix             IPv6 prefix
  tunnel                     NAT64 provisioning tunnel
  version                    draft-ietf-softwire-map-03
```

**`rule-server` で配布サーバの URL を指定できる。**
つまり**ラボに模擬サーバを立てれば、`jp01` 経路を検証できる。**

なお `show nat64 provisioning` / `debug nat64 provisioning` は存在しない。
状態を追うなら `debug nat64 info` / `warn`。

#### 案件に直結する知見

**① OCN バーチャルコネクトは IOS XE 17.6.2 以降。**顧客機 17.9.2a は条件を満たす。

**② Root CA 証明書(Starfield Services Root CA – G2)の事前登録が必要。**
入れ忘れると HTTPS でルール配布サーバに繋がらず、**設定は入っているのに IPv4 が通らない。**
切替当日の事故として出る。**「ルート CA 構成は将来変更の可能性あり」**という運用リスクもある。

**③ `api-key` という設定項目がある。**VNE から API キーを受け取る必要があるかもしれない。
**契約時に何を受け取るのかを確認する項目**が 1 つ増えた。

**④ 「設定を入れた → すぐ通信できる」ではない。**
ルール取得(HTTPS)が終わるまでのタイムラグがあり、その間の切り分け手順が当日の作業に要る。

#### 教訓

> **CLI の `?` で構造は分かるが、それが「正しい使い方」かは分からない。**
> 5 回クラッシュさせて「機種が使えない」と結論しかけたが、**測っていた経路自体が
> ドキュメントに無いものだった。**ユーザーの「Cisco 公式でサポートしてるって言ってるじゃん」
> という指摘がなければ、そのまま誤った結論(RTX への切り替え)を案件に持ち込んでいた。
>
> **一次情報を最初に読む。**サイクル 9 で自分で書いた教訓を、その日のうちに再び踏んだ。

#### 未確定のまま残すこと

- **実回線(jp01 が正常動作する環境)で C1111 が使えるか** — 模擬サーバができるまで不明
- 手動 BMR 経路のクラッシュが Cisco 的に「不具合」か「未サポート構成」か
- C1111-8P が MAP-E の qualify 対象プラットフォームか
  (Cisco Feature Navigator で要確認。会社の CCO アカウントが必要)

### サイクル 11: ルール配布サーバを建てる — **ラボの設計不良の是正**(2026-08-13 深夜)

**きっかけ**: サイクル 10 で「測っていた経路が本番と違った」と分かった後、
ユーザーから「**そもそも本番と違うような環境でやってんかわれ**」という指摘。
**これは個別の設定ミスではなく、ラボの設計そのものの欠陥だった。**

#### 何が欠けていたか

本番の MAP-E では、CPE は MAP ルールを**手で設定されない**。
**プロビジョニングサーバから取得する。**
ところが **ラボにはそのサーバが存在しなかった**。

そのため MAP-E の検証は、最初から一貫して「CPE に BMR を手書きする」経路で行われていた。
OpenWrt がその方式で動くため、成立しているように見えていた。
**本番に存在する構成要素が丸ごと欠けていることに、サイクル 3 から 10 まで誰も気づかなかった。**

| | ラボがやっていたこと | 本番で起きていること |
|---|---|---|
| ルールの入手 | **人間が CLI で手書き** | **CPE が DNS で発見 → HTTPS GET → JSON で取得** |
| ルール配布サーバ | **存在しない** | VNE が運用している |
| CPE の IPv4/PSID | 人間が計算して設定 | **CPE が EA ビットを展開して自力で導出** |

#### Plan

`jp01` の相手方をラボに建て、**本番と同じ経路**で C1111 に MAP-E を通す。
成功条件:

1. 仕様の一次情報を特定する(推測で作らない)
2. CPE が **DNS から自動発見**できる(URL 直打ちに頼らない)
3. HTTPS + 証明書検証まで本番同様に成立する
4. 配ったルールから導出される値が、**ラボの既存設定と一致する**ことを独立に検証する

#### Do

**① 仕様は公開されていた。** 推測で作らずに済んだ。

`nat64 provisioning mode jp01` の相手方は、JAIPA(日本インターネットプロバイダー協会)
「IPv6 マイグレーション技術の国内標準プロビジョニング方式」= **HB46PP**
(HTTP-Based IPv4 over IPv6 Provisioning Protocol)。
**仕様書が GitHub で公開されている**: <https://github.com/v6pc/v6mig-prov/blob/master/spec.md>

流れ:

```
CPE --- TXT? 4over6.info ---> DNS
CPE <-- "v=v6mig-1 url=https://vne.example.jp/rule.cgi t=b"
CPE --- GET /rule.cgi?vendorid=..&product=..&capability=map_e,dslite ---> サーバ
CPE <-- 200 JSON { map_e: { br, rules: [{ipv6, ipv4, ea_length, psid_offset}] } }
CPE --- 自分の委譲プレフィックスから EA ビットを展開して IPv4/PSID を導出
```

**②** `lab/ipoe/inet/ruleserver.py`(HB46PP サーバ)と
`lab/ipoe/inet/setup-ruleserver.sh`(構築)を作成。
`lab/ipoe/ce/hb46pp-client.py`(**CPE 側の参照実装**)も作成。

**③** INET-SIM に構築。`2001:db8:cafe::a1` / DNS TXT / ラボ CA / systemd。

#### Check

| # | 確認したこと | 結果 |
|---|---|---|
| ① | DNS TXT の発見 | **OK** `dig -t TXT 4over6.info` が `v=v6mig-1 url=https://prov.lab.example/rule.cgi t=b` を返す |
| ② | HTTP での取得 | **OK** 8080 で JSON を返す |
| ③ | HTTPS + 証明書検証での取得 | **OK** ラボ CA で検証して成功 |
| ④ | 要求の捕獲 | **OK** リクエスト行・全クエリパラメータ・全ヘッダをログに残す |
| ⑤ | **導出値の独立検証** | **OK** 下表 |
| ⑥ | NGN が広報する DNS が同じ dnsmasq か | **OK** `RDNSS 2001:db8:cafe::53` = TXT を持たせた dnsmasq と同一。**CPE は本番同様に自動発見できる** |
| ⑦ | NGN からサーバへの経路 | **OK** `2001:db8:cafe::/64 via 2001:db8:ff00::2`(VNE 経由)が存在 |

**⑤ の中身** — 配布した BMR(`2001:db8:1000::/40` / `198.51.100.0/24` / ea_length 16 /
psid_offset 4)から参照実装が導いた値と、ラボの既存設定の突き合わせ:

| モード | 委譲プレフィックス | 共有 IPv4 | PSID | MAP CE アドレス | ポート数 |
|---|---|---|---|---|---|
| PD | `2001:db8:100a:500::/56` | 198.51.100.10 | 5 | `2001:db8:100a:500:0:c633:640a:5` | 240 |
| RA | `2001:db8:1014:300::/64` | 198.51.100.20 | 3 | `2001:db8:1014:300:0:c633:6414:3` | 240 |

**両方とも `setup-map-br.sh` / `radvd.conf` に書かれていた値と完全一致。**
= 配っているルールは正しく、CPE 側の導出も正しい。

#### 踏んだこと

| 症状 | 原因 | 対処 |
|---|---|---|
| `Address already in use` で起動しない | **nginx が `listen [::]:80` でワイルドカード待受**しており、専用アドレスを足しても IPv6 の 80 番は取れない | HTTP を 8080 に。HB46PP の TXT は URL をそのまま配れるので問題にならない |
| `deploy.sh vms` で直したはずのスクリプトが古いまま | `vms` は**ホスト上のコピーを配るモード**で、作業機の編集は載らない | フル `deploy.sh` を使う |
| `dig @127.0.0.1` が空振り | dnsmasq は `bind-dynamic` + `interface=<INET側>` で**ループバックでは待ち受けていない** | `@203.0.113.53` へ投げる |

#### Act

- HTTP(8080)と HTTPS(443)を**常時両方**上げる作りにした。
  Cisco の `rule-server` が `http://` を受けるか未確定なので、朝に建て直さず両方試せる
- 応答 JSON は**リクエストのたびに読み直す**。ルータの反応を見ながら即座に書き換えられる
- リポジトリ側の JSON には説明用の `_comment_*` キーを置き、**配置時に落として**
  厳密に仕様どおりの JSON にする
- 手順書を [mape-provisioning.md](mape-provisioning.md) に作成(モードを明示した手順 A〜N)

#### この検証で決まること

| 結果 | 意味 | 次の判断 |
|---|---|---|
| ルールを取得して転送できる | 落ちるのは**手書き BMR 経路だけ** | C1111-8P で郡山の案件に進める |
| 同じように QFP が落ちる | **MAP-E のデータパス自体**が壊れている | YAMAHA 等への切り替え材料 |
| 要求が来ない/形式が違う | Cisco の `jp01` が HB46PP と別物 | 捕獲ログから応答を作り直して再試行 |

#### 教訓

> **「動いた」は「本番と同じ道で動いた」ではない。**
> ラボは 10 サイクル回して PASS を積み上げていたが、MAP-E に関しては
> **本番に存在しない経路**を測り続けていた。個々の手順が正しくても、
> **構成要素が欠けていること自体は、手順の検証では絶対に見つからない。**
>
> 気づけたのは、実機が落ちて一次情報(Cisco のドキュメント)を読み直したから。
> **「ラボに無いもの」は「本番にも無いもの」だと無意識に仮定していた。**

#### 未確定のまま残すこと

- **Cisco の `jp01` が HB46PP そのものか。** サブモードに `api-key` があるが、
  HB46PP の認証は `user`/`pass`/`token` で `api-key` は無い。JPNE 系の独自 API の可能性。
  **実機の要求を捕獲すれば確定する**(手順 K)
- `service-prefix` / `tunnel` サブコマンドの意味
- C1111-8P が MAP-E の qualify 対象プラットフォームか(Cisco Feature Navigator / CCO 必要)

---

### サイクル 12: C1111-8P で本番経路の MAP-E が通った(2026-08-14)

**結論から: C1111-8P は使える。郡山の医療機関案件はこの機種で進められる。**

サイクル 9 の「手動 BMR で 6/6 クラッシュ」は、**本番に存在しない経路での結果**だった。
ルール配布サーバから取得する正規の経路では、**落ちずに、実トラフィックが通った。**

```
PC → curl http://203.0.113.80/
  src 198.51.100.20      ← MAP-E の共有 IPv4。導出値と一致
```
...
| # | 確認したこと | 結果 |
|---|---|---|
| ① | ルータが出す要求の形 | **Cisco 公式ドキュメントの OCN 仕様と完全一致** |
| ② | 応答形式の特定 | `basicMapRules` + **全フィールド文字列** + `application/json; charset=utf-8` |
| ③ | ルータがルールを受理 | `show nat64 map-e` に BMR が入った |
| ④ | 導出値 | **参照実装の計算と完全一致**(下表) |
| ⑤ | CE の MAP アドレス | **RFC 7597 と違う並びだった**(後述。ラボの BR を張り替えて解決) |
| ⑥ | 実トラフィック | **通った**。QFP は落ちなかった |
| ⑦ | 割当ポートの遵守 | 送信元ポート **4145** = 割当範囲 4144-4159 の中 |

**① ルータが実際に喋った内容**(ラボのサーバで捕獲):
...
Cisco の 15M&T ドキュメント「Map Rule Server URL Specification」の
`https://rule.map.ocn.ad.jp/?ipv6Prefix=<address>&ipv6PrefixLength=<prefixLength>&code=<API Key>`
と**同じ形**。`api-key` が `code=` として乗る。**実機に喋らせなければ確定できなかった。**

**④ 導出値の照合**(`show nat64 map-e` の実測 vs `lab/ipoe/ce/hb46pp-client.py` の計算):

| 項目 | 参照実装 | 実機 |
|---|---|---|
| Share-ratio | 256 | **256** |
| 連続ポート数 | 16 | **16** |
| Port-offset-bits | 4 | **4** |
| **PSID** | **3** | **3** |
| 共有 IPv4 | 198.51.100.20 | **198.51.100.20** |
| ポート数 | 240 (15×16) | 15×16 = **240** |

#### 判明した応答フォーマット(一次情報が無く、実機で確定させた)
...
- **値は全部文字列**。数値で返すと、ルータは domain を作るところまで行って
  **BMR を黙って破棄する**(`Creating new MAP-T ... for domain 1000` の直後に
  `Delete BMR data for domain 1000`)。エラーは一切出ない
- フィールド名は Cisco ドキュメントの「Map Rule Server Response Parameters」表どおり。
  **器の名前 `basicMapRules` だけは一次情報が無く**、実機が受理したことで確定した
- `Content-Type: application/json; charset=utf-8`

#### ⑤ CE の MAP アドレスが RFC 7597 と違う — **ラボの BR が噛み合わなくなる**

| | インタフェース ID | アドレス |
|---|---|---|
| RFC 7597 5.2 | `0000:<IPv4>:<PSID>` | `2001:db8:1014:300:0:c633:6414:3` |
| **実機(draft-03)** | `00:<IPv4>:<PSID>:00`(1 バイト右にずれる) | `2001:db8:1014:300:c6:3364:1400:300` |

中身(IPv4=198.51.100.20 / PSID=3)は同じだが**並びが違う**。
ラボの BR はトンネル終点を静的に持つので、**RFC 7597 の方に張ったままだと IPv4 が全断する。**
参照実装が両方を出すようにし、自己検証にも実機の値を組み込んだ。

> **実機を繋がなければ絶対に出てこない差分。**机上の RFC 読解では RFC 7597 の方しか出てこない。

#### 取得の引き金(実測)

- **設定が揃った瞬間には飛ばない。**`service-prefix` を入れた後、
  **WAN のアドレス再取得**(`shutdown`/`no shutdown`)で初めて飛んだ
- その後は **約 8 分周期でリトライ**し続ける(13:39 → 13:46 → 13:59 → 14:11)
- 取得したルールは **`bootflash:/mape/mape-rule.json` に暗号化して保存**される。
  中身は読めない(`rule-server 2 ...` と同じ思想。実網の API キーとルールを秘匿する設計)

#### 踏んだこと

| 症状 | 原因 | 対処 |
|---|---|---|
| `nat64 provisioning mode jp01 version ...` が構文エラー | 17.9.5f では `version` は**サブモード内**。ドキュメントの 1 行表記は古い | サブモードで `version` を入れる |
| `api-key LABTEST` が入らない | **12〜80 文字**必要 | 12 文字以上にする |
| **`ping` は飛ばないのに UDP は通る** | **PC が Wi-Fi にも繋がっていて、そちらへ抜けていた** | Wi-Fi を切る。**切替当日に必ず踏む**ので手順書に追記 |
| BR に届いていないように見えた | **こちらの tcpdump のインタフェース名の間違い**(VNE は `eth1` ではない) | NGN 側で捕獲し直して解決 |
| 1 回だけ `Process = HTTP CORE` で Segfault | **同じ入力で再現しなかった**(次の周期では正常に受理) | 下記 |

#### 再現しなかったクラッシュ(記録として残す)

4 回目の取得で **IOSd の `HTTP CORE` プロセスが Segmentation fault** し、
再起動時に一度ブートにも失敗した(その後は正常起動、イメージも無傷)。
**まったく同じ応答で 5 回目は正常に受理**したので、決定的なバグではなく
初回受信時のレースと見られる。ただし Segfault は Segfault なので、
`bootflash:core/CPE_RP_0-system-report_20260814-230256-JST.tar.gz` を証拠として残す。

**サイクル 9 の 6 回とは落ちた場所が違う**:

| | サイクル 9(6/6) | サイクル 12(1 回) |
|---|---|---|
| 落ちた場所 | **QFP**(転送エンジン) | **IOSd の HTTP CORE** |
| きっかけ | LAN のトラフィック | 応答の受信 |
| 経路 | 手動 BMR(ドキュメントに無い) | jp01(正規) |
| 再現性 | **6/6** | **1/2** |

#### Act

- `ruleserver.py` を **要求の形で jp01 / HB46PP を自動判別**する作りにした。
  `ipv6Prefix` があれば jp01、`vendorid` があれば HB46PP。サーバの建て分けが要らない
- 応答は `response-jp01.json` / `response-hb46pp.json` の 2 本立てに分離
- 参照実装に draft-03 のアドレス計算を追加し、**実機の値で自己検証**するようにした

#### 教訓

> **「ドキュメントに書いていないから」で止めなくてよかった。**
> 応答フォーマットの器の名前は一次情報に無く、third-party の記事から拾った推測だった。
> それでも **実機に投げれば当たりかどうかは分かる**。ラボにサーバを建てたことで、
> 「推測を実機で検証する」ループが回せるようになった。
>
> **そして、サイクル 9 で出しかけた「この機種は使えない」という結論は完全に誤りだった。**
> 誤っていた理由は一つで、**本番に無い経路を測っていたから**。

#### 未確定のまま残すこと

- **実回線の OCN で同じ形式が返るか。**器の名前 `basicMapRules` は実機が受理したので
  「ルータがこの形を期待している」ことは確定したが、**OCN が実際に返す JSON は未確認**
- C1111-8P が MAP-E の qualify 対象プラットフォームか(Cisco Feature Navigator / CCO 必要)
- **240 ポートを使い切れるか**(バックログ 10)。割当範囲内のポートを使うことは確認できたが、
  15 レンジ全部を使うかは未測定

---

### サイクル 12 の後始末: 勉強会資料への反映(2026-08-15)

サイクル 11/12 の実測を説明会資料とハンズオン資料に反映した。

**顧客向け資料に誤った記述が載っていた**ので、まずそれを訂正した。

| 資料 | 直したこと |
|---|---|
| 説明会 2-5 | 「C1111-8P は LAN を繋いだ瞬間に落ちる」→ **本番経路では通る**に訂正 |
| 説明会 3-3b | **新規。**「計算のルールはどこから来るのか」= 自動設定の正体 |
| 説明会 3-5 | VPN を「机上の整理」→ **何が確実で何が未測定か**を分けて明示 |
| 説明会 4-4b | **新規。**C1111-8P の MAP-E 実測表 |
| 説明会 4-5 ④ | **新規。**「動いた」は「本番と同じ道で動いた」ではない |
| ハンズオン 演習 5b | **新規。**ルール配布サーバの実演 + 配る値を変えて見せる |
| ハンズオン 演習 6 | **MSS clamp が Vlan1 にも入っているのに Tunnel0 しか外していなかった**(再現しない)。両方外す手順に訂正 |

> **説明会 3-3b が長らく抜けていたこと自体が、サイクル 11 の症状だった。**
> 「降ってきた IPv6 から計算で決まる」と説明しておきながら、
> **その計算ルールの出どころを説明していなかった。**
> 検証環境にルール配布サーバが無かったので、書いている側も気づいていなかった。

枚数: 説明会 42 → 45 枚(52 分 → 57 分)、ハンズオン 31 → 33 枚(150 分 → 165 分)。
どちらも削る順番と「削ってはいけない箇所」を資料に明記した。

**レンダラ(LibreOffice)がローカルに無い**ため、視覚 QA は
「既存スライドで実績のある行長・行数と比較して逸脱を洗う」形で代用した
(スクリプトは scratchpad に置いた使い捨て)。新規分はすべて既存の範囲内。

---

### サイクル 13: 固定IP1(share-ratio 1)の実機検証 と ポート枯渇の自然発生(2026-08-16)

**郡山の医療機関案件は固定IP1** なので、共有IPだけでなくこちらも実機で通す必要があった。

#### ① 固定IP1 のルールを実機が受理した

サーバが返す `eaBitLength` を **16 → 8** に変えるだけ(他は同一)。

```
CPE# show nat64 map-e
   Basic-mapping-rule
      Port-parameters
         Share-ratio 1   Contiguous-ports 1024   Start-port 1024
         Share-ratio-bits 0   Contiguous-ports-bits 10   Port-offset-bits 6
      Port-set-id 0
CPE# show ipv6 interface brief
    2001:DB8:1014:300:C6:3364:1400:0     ← PSID 0 になった分アドレスも変わった
```
...
| 配る `eaBitLength` | 16 | **8** |
| Share-ratio | 256 | **1** |
| 使えるポート | 240 | **64512**(1024〜65535) |
| 共有 IPv4 | 198.51.100.20 | **198.51.100.20**(同じ) |
| CE の MAP アドレス | `...1400:300` | **`...1400:0`** |

**IPv4 は変わらず、ポートの持ち分だけが変わる。**「固定IPオプション」で契約上
変わっているのは、**サーバが返すこの数字ひとつ**だということが実機で見えた。

#### ② Cisco ドキュメントの記述が実機で裏付けられた

> If the share ratio is 1, there are no port-offset bits as the value is
> automatically set to 6 and the start port is set to 1024.

**こちらは `psIdOffset 0` を配ったのに、ルータは 6 に上書きした。**
結果、全 65536 ではなく**ウェルノウンポートを除いた 64512** になる。
参照実装は 0 のまま計算して 65536 と出していたので、**実機に合わせて修正した**
(`hb46pp-client.py` に share-ratio 1 の読み替えを実装。自己検証も 3 パターンに)。

#### ③ 再取得の引き金(重要)

**一度ルールを受理すると、キャッシュを消してもインタフェースを上下させても取りに来ない。**

- サイクル 12 で 8 分周期のリトライが回っていたのは、**ルールを受理できず失敗し続けていた**から
- 受理後は再取得しない。`delete bootflash:/mape/mape-rule.json` + WAN の shut/no shut でも駄目だった
- **確実な引き金は reload。**Cisco ドキュメントの「CE を初めて起動したとき要求を送る」がそれ

> **実案件で「ルールが変わったのに反映されない」ときは、再起動が正解。**
> 設定を入れ直しても効かない。

#### ④ ポート枯渇が「自然に」起きた — R3 の実機裏付け

検証を放置した夜間、**仕込んでいないのにポート枯渇が発生した**。

```
%NAT-6-ADDR_ALLOC_FAILURE: Address allocation failed; pool 1 may be exhausted [1]
  port range: HIGH, non-PATable: NO, for ALG: NO, input intf: Vlan1,
  created by pkt: src_ip 192.168.100.50 dst_ip 203.0.113.53 src_port 60125 dst_port 53 proto 6
```

**Windows PC を 1 台繋いで放置しただけ**で 240 個を使い切った。
バックグラウンド通信(Update / テレメトリ / OneDrive 等)だけでこうなる。

**説明会資料の「240 個は少ない」という主張の、実機での裏付けが取れた。**
しかも**仕込んだ再現レシピ(R3)より説得力がある**。R3 は「わざと枯らす」手順だが、
これは**放っておいたら勝手に枯れた**という記録である。

> 注: **通信が終わってもポートはすぐ解放されない**(タイマー保持)ので、
> 実際に同時利用できる数は 240 より少ない。

#### ⑤ 踏んだ: **ルールが網の配るプレフィックス長と整合していないと通らない**

固定IP1 のルールを配ったあと、**CPE は送っているのに戻りが 0** になった。

```
CPE# show nat64 statistics
   Vlan1     IPv4 -> IPv6  MAP-E: 1355     ← 出ている
   Gi0/0/0   IPv6 -> IPv4  MAP-E: 0        ← 戻ってこない
```

**原因はこちらが配ったルールの矛盾だった。**

最初に配った固定IP ルールは `ipv6Prefix /40` + `eaBitLength 8` で、
これは「**End-user プレフィックスは `/48`**」という意味になる。
ところがラボの NGN が配っているのは **`/64`**(`2001:db8:1014:300::`)。
**ルールが要求する長さと、網が配る長さが食い違っていた。**

その結果ルータは、**インタフェースには `...1014:300:...` を付けながら、
トンネルの送信元には `...1014:0:...` を使う**という状態になった。
後者は網のどこにも存在しないアドレスなので、返しようがない。

```
実際の送信元: 2001:db8:1014:0:c6:3364:1400:0     ← 網に存在しない
show の表示 : 2001:db8:1014:300:C6:3364:1400:0
```

**正しい固定IP のルールは EA ビットを使わない。**顧客の IPv4 をそのまま名指しする。

| | 誤(最初に配ったもの) | 正 |
|---|---|---|
| `ipv6Prefix` | `2001:db8:1000::/40` | **`2001:db8:1014:300::/64`** |
| `ipv4Prefix` | `198.51.100.0/24` | **`198.51.100.20/32`** |
| `eaBitLength` | `8` | **`0`** |
| End-user プレフィックス | `/48`(網と不一致) | **`/64`(網と一致)** |

直したら**一発で通った**。

```
2001:db8:1014:300:c6:3364:1400:0 > 2001:db8:9999::1:
    IP 198.51.100.20.1025 > 203.0.113.53.53
    IP 198.51.100.20.1093 > 203.0.113.53.53
```

**送信元ポートが 1025 / 1093** — 共有IPのときは 4144〜4159 の 16 個に
閉じ込められていたのが、**1024 番台から自由に使えている**。往復とも成立。

> **固定IP のルールは CE ごとに違う。**「その顧客の委譲プレフィックスと IPv4」を
> 名指しするので、共有IP のルール(CE に依存しない)とは性質が違う。
> 実サービスでもそうなっているはずで、**「固定IPにしたら繋がらない」が起きるとしたら
> この噛み合わせ**だと考えられる。

#### ⑥ 副産物: MAP アドレスの前半は「委譲された /64」ではない
...
アクセス網で捕獲したら、**CPE が使っている送信元アドレスが想定と違った**。

```
実際: 2001:db8:1014:0:c6:3364:1400:0     ← 4 番目のブロックが 0
想定: 2001:db8:1014:300:c6:3364:1400:0
```

**MAP アドレスの前半は End-user IPv6 プレフィックス**で、これは
**ルールから決まる**(ルールの `/40` + EA 長)。委譲された `/64` ではない。

| | End-user プレフィックス | 委譲 `/64` と |
|---|---|---|
| 共有IP (ea 16) | `/40 + 16` = **`/56`** → `2001:db8:1014:300::` | **偶然一致する** |
| 固定IP (ea 8) | `/40 + 8` = **`/48`** → `2001:db8:1014::` | **食い違う** |

**共有IPで動いていたのは偶然だった。**参照実装が委譲プレフィックスで計算していたため、
共有IPでは当たり、固定IPで外した。**実機に合わせて修正済み**
(`hb46pp-client.py` / `lab-mode.sh` の両方)。

> `show ipv6 interface brief` は `2001:DB8:1014:300:C6:3364:1400:0` を表示していたのに、
> **実際の送信元は `2001:db8:1014:0:...` だった。**
> **インタフェースの表示と、トンネルの送信元が一致しない。**
> 表示を信じて BR を張ると外す。**キャプチャで確かめるしかない。**

#### 案件への意味

- **郡山(固定IP1)はポート枯渇の心配なし。**64512 個で PPPoE と同じ土俵
- **共有IPの案件で拠点に PC が数十台ある構成は、それだけで危ない。**
  MAP-E 単独を避けるか、固定IPオプションを見積に入れる判断材料になる

---

### サイクル 14: 「ラボが本番より甘い」を潰す(2026-08-16)

サイクル 11〜13 で分かったのは、**ラボが本番と違う形をしていると、
手順をいくら検証しても気づけない**ということだった。同じ形の甘さが
バックログに 3 つ残っていたので、順に潰す。

**その前に、埋まらないものに線を引いた。**

| 分類 | 内容 |
|---|---|
| **ラボで完結する** | 17 AFTR セッション上限 / 18 切戻せない演習 / 19 BR の入口検査 / 20 accel-ppp 固定 |
| **機材が要る** | 16 HGW 実機(買わないと無理) |
| **人の操作が要る** | 12 Windows と 892FJ の DHCP 相互接続 / 演習の通し実走 |
| **原理的に無理** | **実回線の OCN が返す JSON** / 実 VNE の癖 / ISP の開通処理 / 性能 |

> **「原理的に無理」をバックログに置き続けない。**「いつかやる」ように見えてしまう。
> README の「できないこと」に移し、バックログからは落とす。

#### 19: BR の入口検査 (map-enforce) を実装し、既定 ON にした — **完了**

**それまでの状態**: `test-matrix.md` に**手打ちの nft レシピ**が書いてあるだけで、
しかも**ポート範囲が 1 本しか書かれていなかった**(本来 15 本)。実 BR の代わりに
なっていない。**ラボの BR は事実上何でも通していた。**

`setup-map-br.sh` に組み込み、**既定 ON** にした。

- ポート集合は **CE の MAP アドレスから PSID を取り出して**計算する
  (BR と CE で PSID がズレる事故を防ぐため)
- 落とした通信は `MAP-ENFORCE-DROP:` として kernel ログに出る
- `MAP_ENFORCE=0` で切れる。`stop` でテーブルも消える

**実測で確かめた 2 つ**:

| 試したこと | 結果 |
|---|---|
| 正しい CE(固定IP・1024〜65535 を使用)に対して有効化 | **drop 0 件。**正常な通信は落とさない |
| 許可範囲だけ共有IP用(4096番台の 240 個)にすり替え | **20 秒で 46 パケットを遮断** |

```
MAP-ENFORCE-DROP: IN=map0 SRC=198.51.100.20 DST=203.0.113.53 PROTO=UDP SPT=1024 DPT=53
```

送信元ポート 1024 が許可範囲の外なので落ちている。**実 BR と同じ振る舞い。**

**踏んだこと**: nft のチェーン名に `fwd` は使えない(予約語と衝突して
`syntax error, unexpected fwd`)。`check` に改名して解決。

**分かったこと**: ICMP はポートを持たない(識別子で分ける)ので、ここでは検査しない。
実 BR は ICMP 識別子も見るが、**ラボで落とすと切り分けが濁る害のほうが大きい**と判断した。
...
#### ラウンド 4: 8 エージェント / CONFIRMED 6 (修正 3 + 限界 3)

またもラウンド3の修正が新バグを生んだ。`!= "fixed"` の分岐が **MISMATCH を
「fixed でない」側に落とし**、MISMATCH から mape を再実行すると reconcile を素通りして
警告が黙って消えていた。**分岐設計が 2 連続で穴を生んだため、分岐自体を減らす**
方向に倒した (INET があれば毎回冪等に reconcile)。prov rule の部分失敗も
中間 MISMATCH で真実を指すようにした。

**限界として記録** (構造的・修正対象外):
- rule 状態は自己申告の履歴。state ファイルを消すと痕跡ごと消える
- reconcile は exit code を信頼し読み戻しをしない。lab-mode を介さない手動変更は追跡不能

#### ラウンド 5: 8 エージェント / CONFIRMED 6

ラウンド4の「未構築なら exit 0」が **prov rule 経路で「配っているつもりで何も
配っていない」嘘の成功**を作っていた (r4 以前は die していた)。構築フローが
ルールを shared に初期化するのに state が追随しない矛盾も検出。
→ 未構築時の挙動を文脈で分離 (RULE_IF_BUILT)、prov on で state 追随、
破壊的上書きの告知復活、中間 MISMATCH を悲観側 (rsh の前) に移動。
sudo 経由の環境変数伝搬は実 VM で printenv により確認。

**発見はラウンド4以降 lab-mode の状態管理レイヤに収斂し、データパス
(enforce / derive / 配布応答 / ランブック) はラウンド3以降無傷。**

#### ラウンド 6〜7 と、その後

ラウンド 6 で「記帳(state に書く)方式そのものが繰り返しズレを生む」と判断し、
**記帳をやめて `status` が表示のたびに実測する方式に交換した。**
BR は構成成功時に `/run/map-br.state` を書き、`status` はそれとサーバの配布 JSON を
読んで「一致 / 不一致 / 確認不可」を出す。ラウンド 7 では実測の**範囲**を広げた
(サービスの生死・PSID・到達可否を測る)。

**この監査で直したもの**

| 対象 | 内容 |
|---|---|
| `setup-map-br.sh` | 入口検査の PSID を明示指定に (アドレスからの抽出をやめた) |
| 同上 | 失敗時にトンネルを残さない順序 (fail-closed)。失敗時はマーカーも消す |
| `lab-mode.sh` | `status` を実測方式に。サービス生死・PSID・到達可否まで見る |
| 各ドキュメント | RA 切替コマンドに `CE_PSID=3` を追記 (省くと BR が CE を落とす) |
| `hb46pp-client.py` | BR 起動コマンドの案内を draft-03 / RFC 7597 の両方 + ポート集合 3 値に |

**まだ手を入れていないところ**

- `lab-mode.sh` の状態表示まわりは、監査を打ち切った時点でも指摘が出続けていた。
  データパス (入口検査・導出・配布応答) はラウンド 3 以降指摘なし
- 監査の停止条件 (2 ラウンド連続で新規指摘ゼロ) には達していない

**教訓**

> **修正は 5 ラウンド連続で新しい不具合を生んだ。**パッチを重ねるより、
> 「記帳をやめて実測にする」のように**設計を替える**方が早く収束した。
>
> **そして監査に集中している間、runbook を一度も更新していなかった。**
> ラボが本番と違うことを探していた当人が、**手順書が実装と違う**状態を作っていた。
> 指摘されるまで気づかなかった。**成果物の更新は、検証と同じ回で閉じること。**
...
| 10 | **OpenWrt の MAP-E が実効 16 ポートしか使えない**(nft の snat が終端判定で、15 本のルールが同じマッチ条件のため先頭ブロックしか使われない)。ラボ固有の癖か OpenWrt のバグかは未確認。実機 CPE では NAT エンジンがポート集合を正しく扱うはずなので、**実機検証時に必ず数え直すこと** | 中 | **未解決(サイクル 7・9)**。892FJ は MAP-E 非対応。CML の Cat8000v は**設定構文と導出値は設計と完全一致した(240 = 15 × 16)**が、**MAP-E を有効にすると転送エンジン(QFP)が落ちて再起動**するため転送を測れなかった。**実機の IOS XE か RTX 等の別 CE が必要。**演習では「設計上 240」とだけ言い、「実測で 240 使えた」とは言わないこと。**サイクル 12 で前進**: C1111-8P(jp01 経由)で実トラフィックが通り、**送信元ポート 4145 = 割当範囲 4144-4159 の中**を使うことを実測した(= CE がポート集合を守っている)。**サイクル 13 で決着**: Windows PC を 1 台繋いで放置しただけで `%NAT-6-ADDR_ALLOC_FAILURE ... pool 1 may be exhausted` が**自然発生**した。**240 個は 1 台の PC のバックグラウンド通信で使い切れる**ことが実機で確定。OpenWrt の「16 個しか使えない」はラボ固有(nft の癖)で、**実機の NAT エンジンはポート集合を正しく使う**ことも併せて確認できた |
...
| 14 | **拠点間 IPsec VPN が MAP-E / DS-Lite 配下で張れるかをラボで検証する。**説明会資料 §3-5 に「発信なら張れる(両端が NAT-T 対応であること)/待ち受けは不可」と**机上の整理**を載せたが、**実測していない**。模擬勉強会で現場担当役から「『たいてい張れます』ではお客様に言えない。期日のない宿題は宿題じゃない」と指摘された。確認したいのは ① MAP-E 配下からの発信で NAT-T が通るか ② 対向(固定 IP 側)に設定変更が要るか ③ 892FJ + DS-Lite の発信で張れるか ④ 両端 MAP-E で本当に張れないか | 低 | **優先度を下げた(サイクル 12 後)**。理由: ① **IPsec は IPv6 でなく VPN の領域**で、ここを開けると SIP・監視カメラ・ゲームと際限がなくなる ② 結論は原理から確定できる — **待ち受け不可**(IKE の UDP 500/4500 は PSID で決まる 240 ポートに入らない)と**生 ESP 不可**(ポートが無く NAPT できない)は確実 ③ **発信側の裏付けはサイクル 12 で取れた** — MAP-E 配下から `198.51.100.20:4145`(割当範囲内)で UDP を出して応答が戻ることを実測済み。NAT-T はこの上に乗るだけ。**資料は「机上の整理」から「設計上の帰結 + UDP 発信の実測」に格上げできる。**実際に IPsec を張るのは**案件で必要になったとき**でよい |
...
| 22 | **`jp01` のルール配布サーバを模擬する。**`nat64 provisioning mode jp01` のサブモードに `rule-server`(URL 指定)と `api-key` があるので、ラボに HTTPS の模擬サーバを立てれば **MAP-E の正規経路を検証できる**。実回線に出る前に「C1111 で OCN バーチャルコネクトが動くか」を確定させられる。**手動 BMR は 5/5 でクラッシュし、ドキュメントにも無い経路**なので、こちらでしか判定できない | **最高** | **サーバ側は完了(サイクル 11)。ルータ側の実施は未了。** 仕様は JAIPA の HB46PP で**公開されていた**(<https://github.com/v6pc/v6mig-prov>)。`lab/ipoe/inet/setup-ruleserver.sh` で構築、DNS TXT 発見・HTTP・HTTPS(ラボ CA で検証)・要求の捕獲まで実測済み。導出値もラボの既存設定と一致を確認。手順は [mape-provisioning.md](mape-provisioning.md)。**Root CA は実回線では Starfield Services Root CA – G2 が必要だが、ラボでは自前 CA を使う** |
| 19 | **BR の入口検査(`map-enforce`)を既定 ON に格上げする。**ラボの BR は何でも通すので、CE 側の導出誤りや割当ポート外の送信元が**顕在化しない**。実 BR は落とすため「ラボで PASS した CE が現場で弾かれる」が起こり得る。`setup-map-br.sh` に `enforce` サブコマンドを足し、MAP-E の実機検証手順では既定 ON にする | 中 | **完了(サイクル 14)**。`setup-map-br.sh` に組み込み、**既定 ON** にした。ポート集合は CE の MAP アドレスから PSID を取り出して計算する。実測で ① 正しい CE は drop 0 件 ② 許可範囲をずらすと 20 秒で 46 パケット遮断、の両方を確認。落ちたものは `MAP-ENFORCE-DROP:` として kernel ログに出る |
...
| 23 | **固定IP1 相当(share-ratio 1)のルールを実機で検証する。**`lab-mode.sh prov rule fixed` で配れるようにしたが、**C1111-8P で通していない**。導出は参照実装で確認済み(同じ IPv4 `198.51.100.20` のまま PSID 0 / 全ポート、CE の MAP アドレスは `...1400:0` に変わる)。Cisco のドキュメントに「share ratio が 1 のときは port-offset-bits を 6、start-port を 1024 に自動設定する」とあり、`psIdOffset` の扱いが変わる可能性がある | **高** | **完了(サイクル 13)**。実機が受理し、`Share-ratio 1 / Contiguous-ports 1024 / Start-port 1024 / Port-offset-bits 6 / 64512 ポート`。**Cisco の「share-ratio 1 なら offset を 6 に自動設定」も裏付けが取れた**(こちらは 0 を配ったのに 6 に上書きされた)。参照実装をそれに合わせて修正済み。**郡山案件はポート枯渇の心配なし** |
