...
**結果**: **半分達成。**サイクル 7 の疑問には答えが出たが、**転送は未成立のまま**。

#### 分かったこと(いずれも実機で確認)

**① BMR を有効化してもクラッシュしない ← サイクル 7 への答え**

Cat8000v(CML)は `port-parameters` を受理した時点で QFP がクラッシュした。
C1111-8P は**5 要素すべてを揃えて BMR を有効化しても落ちなかった。**

> **ただし「実機なら MAP-E が動く」とは言えない。**下記のとおり転送は成立していない。
> 言えるのは**「Cat8000v のクラッシュは仮想固有だった」**ということだけ。
...
1. **`local-ipv4-prefix` の意味**。いま「この CE の共有 IPv4」(`198.51.100.10/32`)と
   解釈しているが、**「CE の内側 = LAN」(`192.168.100.0/24`)** の可能性がある
2. **NAPT44 が別途必要**。MAP-E の CE は本来
   ① 私設 IPv4 → 共有 IPv4 への NAPT44 → ② IPv6 カプセル化 の 2 段階。
   Cisco がこれを別機能として実装していて、`ip nat` の設定が別に要る可能性
3. **Cisco の MAP-E コンフィギュレーションガイドを当たる。**
   ここから先は当てずっぽうの組み合わせ試行になるため、**一次情報を読むべき**

> **教訓**: パラメータの意味が確定しないまま組み合わせを試すのは、時間の使い方として悪い。
> **CLI の `?` で構造は分かるが、意味は分からない。**次回は先にドキュメントを読む。
