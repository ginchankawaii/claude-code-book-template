...
| 5 | トラブル再現レシピ R1/R3/R4/R5 を実走して再現条件を確定 | **完了**(R1/R3/R4/R5 の 4 件) | 約 80 分 | **4 件とも「レシピのままでは想定と違った」**。R3 は 240 でなく 16 で詰まる / R4 は待受の対照実験が無いと誤判定する / R5 は MSS clamp と PMTU キャッシュの 2 つを外さないと再現しない |
...
**⑥ 方式・モードを何度も切り替えた後は、`run-checks.sh` が収束するまで 2〜3 回かかる(未解決)**
...
**当面の運用**: エビデンスを採る前に **2 回流して 2 回目以降を採用する**。
1 回目で FAIL が出ても、2 回目で消えるなら環境の整定であって障害ではない。
**サイクル 4 の実機検証前に切り分けること**(バックログ 11)。
...
| 11 | **方式・モードを往復させた後、`run-checks.sh` の IPv6 系が収束するまで 2〜3 回かかる**(7→8→11)。IPv4 は初回から安定。近隣探索を疑ってウォームアップを入れたが解消せず、原因未特定。当面はエビデンス採取前に 2 回流す運用 | 中 | サイクル 4 の実機検証前に切り分ける |
