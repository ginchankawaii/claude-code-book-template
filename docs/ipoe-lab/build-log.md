...
| 22 | **`jp01` のルール配布サーバを模擬する。**`nat64 provisioning mode jp01` のサブモードに `rule-server`(URL 指定)と `api-key` があるので、ラボに HTTPS の模擬サーバを立てれば **MAP-E の正規経路を検証できる**。実回線に出る前に「C1111 で OCN バーチャルコネクトが動くか」を確定させられる。**手動 BMR は 5/5 でクラッシュし、ドキュメントにも無い経路**なので、こちらでしか判定できない | **最高** | **未着手**(サイクル 10)。Root CA(Starfield Services Root CA – G2)の登録も併せて必要 |
