...
## 1.5 自社で実際に起きた事故(2 回連続で開通できなかった件)

**このラボを作る動機になった、実在の案件です。**顧客名は伏せます。

### 何が起きたか

回線は **WAKWAK光 IPoEビジネス ワイドプラス**(NTT ME)。**2 回続けて開通できませんでした。**

| | 担当者が想定したもの | 実際 | 結果 |
|---|---|---|---|
| **1 回目** | **動的 IP コース**だと思って設定 | **固定 IP コース**だった | 失敗 |
| **2 回目** | 固定 IP と気づき、**PD 方式**だと思って設定 | **RA 方式**だった | 失敗 |

**2 回失敗して大きな問題になりました。**

### なぜ起きたのか — 担当者のミスとして片付けてはいけない

**公開情報だけでは、机上で決められないからです。**
公式ページ(`https://www.wakwak.com/biz/course/ipoe/wideplus/index.html`)の記載を確認しました。

| 項目 | 公開ページの記載 |
|---|---|
| 動的 IP コース | **transix IPv4接続(DS-Lite)** 対応機種が必要 |
| 固定 IP コース | **transix IPv4接続(固定IP)** 対応機種が必要 |
| **IPv6 の払い出し方式(RA か PD か)** | **記載なし** |
| ひかり電話の有無との関係 | **記載なし** |
| 必要な CPE | 「IPoE(transix)対応ルータが必要」とだけ |

**IPv4 の運び方(方式)は書いてあるのに、IPv6 の払い出し方式が書いていません。**
つまり **RA か PD かは、公開情報からは分かりません。**

### この事故が示している 3 つのこと

**① 「方式」は 1 つではなく 2 段ある**

- **IPv6 の払い出し**(RA 方式 / PD 方式)
- **IPv4 の運び方**(DS-Lite / 固定 IP / MAP-E)

**この 2 つは独立です。**「transix だから DS-Lite」まで分かっても、
**RA か PD かは別に確かめないといけません。**1 回目の失敗は前者、2 回目は後者です。

**② コース名(動的 IP / 固定 IP)で IPv4 の方式が変わる**

同じ「transix」でも、**動的 IP コースは DS-Lite、固定 IP コースは固定 IP 方式**です。
**契約したコースを確認しないと、CPE のテンプレートを間違えます。**

**③ 「ひかり電話ありなら PD」は、事業者サービスでは通用しない**

家庭向けの経験則(ひかり電話あり = PD /56)を、そのまま事業者向けサービスに当てはめると外します。
**事業者ごとに設計が違うので、この案件では固定 IP なのに RA 方式でした。**

### 再発防止(このラボと資料に反映済み)

| やること | 反映先 |
|---|---|
| **契約書で「コース名」まで確認する**(動的 IP か固定 IP か) | 説明会資料 §2-5 |
| **RA か PD かは開通後に実測する。**机上で決めない | 説明会資料 §2-5 / ハンズオン 演習 2 |
| **RA / PD を取り違えたときの症状を、先に見ておく** | **ハンズオン 演習 2** |
| 疎通確認だけで判断しない。**出口アドレスを見る** | 説明会資料 §4-4 / ハンズオン 演習 4 |

**ハンズオンの演習 2 は、この事故をそのまま再現するために作りました。**
`lab-mode.sh pd` に切り替えると、**設定を 1 文字も変えていないのにアドレスが付かなくなります。**
しかも **RA 自体は届いている**ので、現場では原因が見えません。**先に見ておけば気づけます。**
