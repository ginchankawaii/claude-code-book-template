...
> **⚠「Solicit を投げても無応答」の原因は DUID だけではありません。**
> ラボは誰でもすぐ繋がるので、**「無応答 = DUID を疑う」という反射だけが身につく危険**があります。
> 実網では次の順で切り分けてください。
>
> | 順 | 疑うもの | 確かめ方 |
> |---|---|---|
> | ① | **開通が反映されていない** | 開通通知の日時と、ISP / NTT 側の反映完了を確認 |
> | ② | **契約とサービスの突合が NG**(VNE 側に情報が無い) | ISP に「IPv6 接続オプションが有効か」を確認 |
> | ③ | **ひかり電話の契約変更で方式が変わった** | RA / PD が想定と違っていないか**実測**する |
> | ④ | **DUID の種別**(DUID-LLT / EN を送っている) | `tcpdump -ni <ACCESS_IF> udp port 547` |
>
> **①②③ は机上では分かりません。**[research-notes.md §1.5](research-notes.md) の 2 回連続失敗も、
> 原因は DUID ではなく**③(方式の思い込み)と契約コースの確認漏れ**でした。
...
> **⚠ ここで測れるのは「手順と所要時間」だけです。「切戻せること」は測れていません。**
> **ラボの BRAS は無条件・無期限に PPPoE を受け付ける**ので、この訓練は**構造的に必ず成功します。**
> 実案件では **IPoE 開通に伴って ISP 側の PPPoE アカウントが停止される**ことがあり、
> そうなると**手順が正しくても切戻せません。**
>
> **切替前に ISP へ書面で確認してください:**
> 「IPoE 開通後も PPPoE の認証アカウントは有効なままか。切戻し時にそのまま使えるか」
>
> ヒアリングシートの「PPPoE 併用の可否」とは**別の質問**です。併用可でも、
> 切替完了後にアカウントが落とされる運用の事業者があります。
