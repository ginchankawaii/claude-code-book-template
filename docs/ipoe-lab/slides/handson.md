...
**時間**: 200 分(休憩 10 分 × 1 を含む)。**120 分の短縮版**もあります(下記)
...
> ## ⚠ 時間について — 先に読んでください
>
> **模擬開催の実測は 185 分でした**(当時の予定は 150 分)。
> そこへ演習 5b を足したので、**通しでやると 200 分見てください。**
>
> **押したときに削られるのは、必ず「受講者が手を動かす時間」です。**
> 模擬開催では最後の 30 分で司会が自分で打鍵し始め、
> **この研修の設計原則(受講者が触る)が崩れました。**
>
> | | 内訳 |
> |---|---|
> | 導入(構成図・役割の説明) | **10 分**(前回ゼロだったので明示的に確保) |
> | 0-2 実機を消す / 0-3 設定を入れる | 10 + 10 分 |
> | 第1部 アドレスが降りてくるのを見る | 30 分 |
> | 第2部 IPv4 を通す | 55 分(演習 5b 込み) |
> | 休憩 | 10 分 |
> | 第3部 壊す | 45 分(演習 6 の破綻を織り込み) |
> | 第4部 戻して終わる | 10 分 |
> | クロージング(自分の案件でどう使うか) | **10 分**(ここを守ること) |
>
> ---
>
> ### 200 分が取れないとき — 120 分の短縮版
>
> **中途半端に全部やるより、削って最後まで受講者に触らせる方が残ります。**
>
> | やること | 時間 |
> |---|---|
> | 導入 + 0-2 + 0-3 | 25 分 |
> | **演習 1 → 演習 2**(RA 方式 → PD 方式で止まる) | 30 分 |
> | **演習 3 → 演習 4**(DS-Lite を通す → 出口アドレスを見る) | 30 分 |
> | **演習 5b 前半**(MAP-E は誰が設定するのか・司会実演) | 10 分 |
> | 演習 9(戻す) + クロージング | 25 分 |
>
> 落とすのは 演習 5 / 5b後半 / 6 / 7 / 8 です。
>
> ### どちらの版でも削ってはいけないもの
>
> - **演習 2**(PD 方式でアドレスが付かなくなる) — 昨日と同じコマンドで違う結果が出る。
>   模擬開催で受講者 4 人が同時に「え」と言った 10 秒。**この研修の存在理由**です
> - **演習 4**(出口アドレスを見る) — 「通った」で終わらせない癖がつく唯一の場面
> - **演習 5b 前半** — 「MAP-E は誰も設定しない」は説明会で一番腑に落ちないところ
> - **クロージング** — 「自分の案件で使える」で終わらないと、翌日から何も変わりません
...
> ⚠ **ここは `shutdown` / `no shutdown` では効きません。**
> **一度ルールを受理した CPE は、キャッシュを消してもインタフェースを上下させても
> 取りに来ません。**確実な引き金は **reload** です。
> Cisco のドキュメントにも「CE を**初めて起動したとき**に要求を送る」とあります。
> **これ自体が現場で効く知識**なので、受講者にも言ってください。
...
起動を待つ間(2〜3 分)、**この待ち時間を使って**「なぜ再起動が要るのか」を
説明してください。**実案件で「ルールが変わったのに反映されない」ときの答えがこれです。**

起動後に `show nat64 map-e` を見せてください。**`Share-ratio` が変わっています。**
...
| Port-offset-bits | 4 | **6**(ルータが勝手に設定) |
| 使えるポート | **240 個**(4144〜4159 など) | **64512 個**(1024〜65535) |
| 実際の送信元ポート | 4145 | **1025 / 1093 など** |
| 共有 IPv4 | 198.51.100.20 | **198.51.100.20**(同じ!) |

**IPv4 アドレスは同じままで、ポートの持ち分だけが変わります。**
「固定 IP オプション」の正体がこれです。

**違いは「帯を配って計算させる」か「名指しで配る」かです。**
共有IPは `/24` の帯を配って「あなたは何番」を CPE に計算させます。
固定IPは**その顧客の IPv4 を直接**渡すので、計算する余地がありません。

> **ここも指摘してください。**こちらは `psIdOffset` に `0` を配ったのに、
> ルータは **6 に上書き**しました。Cisco のドキュメントに
> 「share-ratio が 1 なら port-offset-bits は 6、start-port は 1024 に自動設定」
> とあるとおりです。だから 65536 個ではなく、**ウェルノウンポートを除いた 64512 個**になります。

> ⚠ **司会が知っておくこと(受講者には言わなくてよい)。**
> **固定IP のルールは CE ごとに違います。**「その顧客の委譲プレフィックスと IPv4」を
> 名指しするからです。ここを共有IPと同じ感覚で `/40` の帯のまま `eaBitLength` だけ
> 変えると、**CPE は網に存在しないアドレスから送信して戻りが一切来なくなります**
> (実際に踏みました。[build-log.md](../build-log.md) サイクル 13 ⑤)。
> `lab-mode.sh prov rule fixed` はモードに応じた正しい値を渡します。

戻すとき(**戻すときも reload が要ります**):
...
### 【受講者】同じ PC で、フラグを 1 つ足して叩く

**`-6` を付けると「IPv4 に逃げるな」という意味**になります。

```powershell
curl.exe -6 -s -o NUL -w "%{time_total}`n" http://www.lab.example/
```

**今度は固まります。**待ちきれなければ `Ctrl-C` で止めてください。

### 出てくる結果

| 打ったもの | 結果 |
|---|---|
| `curl.exe`(そのまま) | **0.2 秒程度。速いので気づけません** |
| `curl.exe -6`(IPv4 に逃げない) | **固まる。数十秒待たされます** |

**同じ PC・同じツール・違いはフラグ 1 つだけ**です。
**環境は壊れたまま**で、逃げ道があるかどうかだけが違います。

> **司会へ**: この形は 892FJ / C1111 のどちらを CE にしても成立します。
> (以前は Linux の `wget` を使う手順でしたが、**実機を CE にすると
> LAB-CLIENT が使えなくなる**ため、受講者の PC だけで完結する形に変えました)
> **開催前に一度、`-6` 付きが実際に固まることを確認しておいてください。**
> 秒数は端末や OS の設定でぶれます。
>
> OpenWrt を CE にしている回なら、Linux の実装差も見せられます:
> `ssh <LAB-CLIENT> 'wget -q -O /dev/null http://www.lab.example/'` → 15 秒でタイムアウト(サイクル 5 実測)
...
**演習 5b をやった回は、配布ルールも戻してください。**

```bash
./lab-mode.sh prov rule shared
```

`status` の「配布ルール」の行が **「共有IP (240 ポート)」** に戻っていることを確認します。
この行は**サーバと BR の両方を実測**しているので、「⚠ 不一致」と出たらもう一度打ってください。

> **固定IP を配ったまま解散すると、次の回の演習 1〜4 が噛み合いません。**
> しかも症状は「なぜか IPv4 だけ通らない」なので、原因に辿り着くのに時間がかかります。
...
- **環境を使いたいときの窓口**: **會津まで。**分からないことは何でも聞いてください
