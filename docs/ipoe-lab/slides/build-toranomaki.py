...
def build():
    ...
    bullet_slide(
        prs,
        "1-4. 演習 1-A: CPE の WAN に何が付いているか  ⚠未検証",
        [
            (0, "手順", "b"),
    ...
    bullet_slide(
        ...
        [
            (0, "ラボ既定ではこの癖は再現しない。既定では DUID-LLT でも PD が降りる", "w"),
    ...
    bullet_slide(
        prs,
        "3-6. 演習 3-A: ポートを数えて、失敗させる  ⚠未検証",
        [
            (0, "① CPE に降りたプレフィックスから共有 IPv4 と PSID を確認し、3-3 の計算と照合", "b"),
            (0, "② ポート集合を紙に書き出し、飛び飛びであることを目で見る (240 個 = 16 × 15)", "b"),
            (0, "③ R3 (ポート制限) を再現する。ただし前提作業が必要", "b"),
            (1, "ラボの BR はポート制限を強制しない (制限は CE の NAT 実装依存)", "w"),
            (1, "BR に enforce 用 nft を投入してから実施する (test-matrix.md R3 に投入と戻し方)", ""),
            (1, "これを飛ばすと演習が成功して終わる = 「MAP-E でも開放できる」と誤学習する", "w"),
            (0, "④ R4: DS-Lite でポート開放を設定し、INET-SIM から nc / ping で叩いて届かないことを確認", "b"),
            (1, "外から叩く手順は test-matrix.md R13 が流用できる", ""),
            (0, "⑤ MTU の確認", "b"),
