...
def build():
    ...
    title_slide(
        ...
        [
            ...
            "検証環境: IPoE 検証ラボ (自宅で構築・実走済み。社内 VMware へ展開予定)",
            "",
        ...
        notes=[
            "【講師へ — 実施前に必ず読むこと】",
            "◆ 使える部分: 教科書との対応表 / 用語集 / つまずきの壁 Q&A (ラボの状態に依存しない)",
            "◆ まだ確定していない部分:",
            "  ・**時間配分は仮案。実測していない。**休憩・質疑の余白が無いので、そのままだと溢れる。",
            "◆ ラボの状態: サイクル 1〜7 実走済み。R1〜R6 / D1 の 7 件も実測完了。",
            "  実機 Cisco 892FJ での DS-Lite も実測済み。runbook-vmware.md は執筆済み。",
            "◆ 配布前にやること:",
            "  ・proxmox-prototype.md §0.5 → §2 → §5 を一巡する",
            "  ・test-matrix §4 の「⚠ 実測」注記を必ず読む (5 件が想定と違った)",
            "  ・**第1回はパイロット実施。**実測が取れるまで第2回以降の日程を確定しない",
            "  ・教科書の節番号・ページ番号を実物と照合する (未実施)",
        ],
    )
    ...
    # 「この教材のステータス」は講師向けの内容しかないので、スライドから外して
    # 表紙のノートに移した (フェーズ5 レビュー P2-21)。投影すると受講者が混乱するため。
    ...
    bullet_slide(
        ...
        footer="手順はラボで実走済み。GUA がどのプレフィックスで付くかは受講者の環境によって変わります",
    )
    ...
    bullet_slide(
        ...
        [
            ...
            (0, "見てほしいこと", "b"),
            (1, "RS → RA が飛ぶか。PD なら Solicit → Advertise → Request → Reply の 4 メッセージ (2 往復)", ""),
    ...
    bullet_slide(
        ...
        "3-0. この回は分量オーバー。絞る",
        [
            (0, "この 3 つは資料を配るだけにします (今日は扱いません)", "b"),
    ...
    bullet_slide(
        ...
        [
            ...
            (1, "docs/ipoe-lab/runbook-vmware.md … 社内 VMware 用 (執筆済み。これ 1 本で構築できます)", ""),
