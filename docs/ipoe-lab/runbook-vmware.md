...
>
> **作業前に構成図([topology.png](topology.png) / [topology.svg](topology.svg))を開いてください。**
> 上の枠がこの手順で作る仮想基盤、枠の下が物理(物理スイッチ・実機 CPE・検証 PC)です。
> §1〜§7 が枠の中、§8 が枠の外(物理)に対応します。

## 会社環境の前提(自宅との違い)

| | 自宅 Proxmox | **会社 VMware** |
|---|---|---|
| CML | あり(Cat8000v の MAP-E CLI は確認済み。**ただし転送は QFP クラッシュで不可 = CE 役には使えない**。サイクル 7) | **なし** |
...
| 工程 | 実測 |
|---|---|
| VM の作成〜起動 | 5 台で約 3 分 + INET-SIM 1 台で 1 分 52 秒 = **6 台で約 5 分**(自宅はスクリプト。VMware は手動のぶん増えます) |
| Linux の setup スクリプト | 4 台で合計 2.6 分(実測)。5 台目の INET-SIM は未計測 |
...
- [ ] **vSwitch の種別を確認**(標準 vSwitch / 分散スイッチ(vDS)/ NSX)。
      **この手順書は標準 vSwitch 前提**です。vDS だとセキュリティ 3 項目の変更権限が
      別ロールになっていることがあり、vDS 6.6 以降では「無差別モード」ではなく
      **MAC ラーニング**を使うのが正解になります。NSX 環境はさらに別物です
...
> ### ⚠ 複数人で実機を触る回(勉強会など)のルール
>
> **このラボのアクセス網は加入者ごとに分離されていません。**実網と違い、
> 全 CE と NGN-SIM / BRAS が**1 つのブロードキャストドメイン**にいます。
> つまり **1 台が WAN 側に RA を撒くと、その場の全員の既定経路が汚染されます**(§9-N)。
> 安価な L2 スイッチには RA Guard がありません。
>
> **どちらかを守ってください:**
> - RA Guard が使える管理スイッチを入れる、または
> - **結線する CPE は常に 1 台だけにする**(そもそも同時検証できる CE は 1 台です)
>
> **壊れたときの復旧**(司会がこれだけ覚えておけば戻せます):
>
> ```bash
> # ① 犯人の CPE で RA を止める (Cisco)
> conf t → interface <WAN> → ipv6 nd ra suppress all
> # ② 網側を張り直す
> ssh <NGN-SIM> sudo ./ipoe/ngn/setup-ngn.sh ra     # または pd
> # ③ 各 CPE で WAN を上げ直す
> #    OpenWrt: ifdown wan6 && ifup wan6
> #    Cisco  : interface <WAN> → shutdown / no shutdown
> ```
...
> ### ⚠ このラボがカバーしていない構成(先に知っておくこと)
>
> **HGW(ひかり電話のホームゲートウェイ)配下の構成は未カバーです。**
> ラボには HGW 相当の機器がありません。**しかし現場の多数派はこの構成**なので、
> 次の事故はラボでは踏めません:
>
> - HGW 側の **IPv6 パケットフィルタの既定値**による遮断
> - **フレッツ・ジョイントで HGW に v6プラスが自動配信され、HGW が勝手に MAP-E を終端**する
>   → 気づかず配下ルータにも設定して**二重終端**になる
> - HGW 配下での PD / RA の癖
>
> **HGW 実機の収容はバックログに積んであります**([build-log.md](build-log.md) §5 の 16)。
>
> **また、Cisco 892FJ は EoS/EoL です。**ラボの検証治具としては有用ですが、
> **現場への提案機種にはしないでください。**
...
> ### ⚠ お客様の LAN は IPv4 のままにするのが通常です(実務の前提)
>
> **PPPoE から IPoE へ切り替える動機は「PPPoE が遅い」(網終端装置の輻輳)であって、
> IPv6 化そのものではありません。** LAN を IPv6 化するとアドレス再設計・FW 見直し・
> アプリ検証が発生するため、**要件でない限りやりません**。
>
> したがって既定の構成は次のとおりです。
>
> | | 実務の既定 | 例外(IPv6 対応が要件のとき) |
> |---|---|---|
> | WAN 側 | IPv6(IPoE)+ IPv4 over IPv6 | 同じ |
> | **LAN 側** | **IPv4 のみ(私設アドレス + NAT)** | IPv6 も配る(PD 方式が前提) |
>
> **この構成では、クライアントから IPv6 が通らないのが正しい状態です。**
> `run-checks` の IPv6 項目を FAIL として数えると誤解を招くので、次を付けてください。
>
> ```bash
> SKIP_V6=1 EXPECT_SRC4=203.0.113.1 ./run-checks.sh      # Linux
> ```
> ```powershell
> .\run-checks.ps1 -SkipV6 -ExpectSrc4 203.0.113.1        # Windows
> ```
>
> なお **RA 方式(ひかり電話なし)では、そもそも LAN に配るプレフィックスがありません。**
> WAN リンクに /64 が 1 つ来るだけで委譲が無いためです。LAN に IPv6 を配るなら
> **PD 方式(ひかり電話あり)が前提**になります。RA 方式で LAN に IPv6 を通したい場合は
> ND プロキシ / パススルーの領域です([test-matrix.md](test-matrix.md) R7)。
...
| `ping 203.0.113.80` | 0% 損失 / **TTL=62** | 2 ホップ減。トンネル経由と整合する(経路の証明は src: のほう) |
...
**セルごとに根拠が違うので [実測] と [調査] を分けて書きます。**
[実測] = 自宅ラボでパケットを流して確認 / [調査] = 資料と CLI の確認のみ。

| 機種 | IPoE(RA/PD) | PPPoE | DS-Lite | MAP-E |
|---|---|---|---|---|
| Cisco 892FJ(classic IOS 15.x) | **○ [実測]**(RA 方式) | ○ [調査] | **○ [実測]** | **×** [調査](classic IOS に CE 機能なし) |
| IOS XE(C1100 系 / Cat8000v) | ○ [調査] | ○ [調査] | ○ [調査] | **△ [実測]**(下記) |
| OpenWrt 24.10.0 | **○ [実測]** | **○ [実測]** | **○ [実測]** | **○ [実測]**(ただし実効 16 ポート。§9-J) |

> **⚠ IOS XE の MAP-E は「CLI はあるが、仮想では動かなかった」が実測結果です。**
> 自宅の CML(Cat8000v / IOS XE 17.15.01a)で `nat64 map-e domain` と
> `nat64 provisioning mode jp01` は通り、`show nat64 map-e` の導出値(共有 IPv4 / PSID /
> ポート数 240)も設計値と完全に一致しました。
> **しかし `port-parameters` を入れた瞬間に QFP(転送エンジン)がクラッシュして転送できません。**
> **実機の IOS XE がどうかは、これでは分かりません。断定しないでください。**
>
> 会社の実機で確認するときは `show version` と `show license boot level` を見たうえで
> `nat64 map-e ?` を打ち直してください(`network-essentials` だと出ない可能性があります)。
> **CLI が通っても、必ずパケットを流して出口アドレスまで確認すること。**
