...
1. handson 演習 6/7/8 を実機で通しで実走し、「出てくる出力」を実測値に置き換える(P0-5/P0-9 とセット)。**2026-08-11 に C1111-8P を購入済み。**892FJ でできない MAP-E 系(バックログ 10 の実効ポート数)もこれで測れる。勉強会は 9 月開催予定
2. ~~lab-mode.sh 修正後の PD/RA × status/mape/restore 実走~~ → **2026-08-11 完了**(build-log 参照。新バグ 1 件も発見・修正)
3. run-checks.ps1 の WAN 断偽陽性テスト(P2-16)
4. ~~『プロフェッショナルIPv6 第2版』の節番号・ページ番号の全数照合~~ → **2026-08-11 完了**(全 45 件。誤り 1 件を訂正)
