...
def load_rules(path):
    ...
    try:
        ...
        raise CheckerError("ルールファイルの解析に失敗しました: %s (%s)" % (path, e))
...
def read_bom(path, col_part=None, col_qty=None, col_desc=None, no_header=False):
    ...
    rows = []
    for i, (raw, lineno) in enumerate(raw_rows):
        if i <= header_idx:
            continue
        ...
        if qty_col is not None and qty_col < len(raw):
            ...
            if qty_col is not None:
                warnings.append("%d行目: 数量セルが無いため1とみなしました" % lineno)
            qty = 1
...
def _read_csv_rows(path):
    ...
    if head.startswith(codecs.BOM_UTF16_LE) or head.startswith(codecs.BOM_UTF16_BE):
        encodings = ("utf-16",)  # Excelの「Unicodeテキスト」保存(タブ区切り)
    else:
    ...
    for encoding in encodings:
        ...
        try:
            return [(row, reader.line_num) for row in reader], encoding
        except csv.Error as e:
            raise CheckerError("CSVとして解析できません: %s (%s)" % (path, e))
    raise CheckerError(
        "文字コードを判定できません(UTF-8/CP932/UTF-16以外?): %s (%s)" % (path, last_error))


def _find_header(raw_rows, col_part):
    targets = {_norm_header(col_part)} if col_part else PART_HEADER_CANDIDATES
    for idx, (row, _lineno) in enumerate(raw_rows[:30]):
...
def _parse_qty(value, lineno, warnings):
    ...
    try:
        ...
        pass
    # 「2台」「$2」「2 pcs」のような表記から数値部分を拾う(黙って1にすると
    # min_qty系ルールの検出漏れにつながるため、読み取れる数は読み取る)
    m = re.search(r"[0-9]+(?:\.[0-9]+)?", s)
    if m:
        qty = int(float(m.group(0)))
        warnings.append("%d行目: 数量 %r から %d を読み取りました(要確認)" % (lineno, s, qty))
        return qty
    ...
    try:
        ...
        if args.out:
            try:
                ...
                raise CheckerError("レポートを書き出せません: %s (%s)" % (args.out, e))
        ...
        return 2
    except Exception as e:  # 実行エラーを「指摘あり(1)」と混同させない
        print("エラー: 予期しない失敗: %r" % (e,), file=sys.stderr)
        return 2
