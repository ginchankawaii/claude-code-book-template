...
- visual（絵に入る描写）は人名・固有エピソード語を含めない
  （graph.static_check_links が新アンカー名込みで静的に遮断する）
"""
...
# 質問ルール
- ノードの技術的性質を日常語に言い換えて聞く（例:「"よそから入ってきたもの"で思い出す自分の体験は？」）
...
- visual: 絵に描く小さな挿絵の描写。**人名・ペット名・固有エピソードの文字は禁止**。一般名詞のイメージだけ
- 回答が曖昧で記憶の実体がない場合は {{"ok": false}} だけを返す（無理にアンカー化しない）
